...
Для успешного запуска сервиса требуется наличие обязательных переменных окружения. Если сервис запускается локально (с профилем `ide`), необходимо создать файл `.env` в корневой папке микросервиса `auth-service`.

Файл `.env` автоматически подтягивается настройками Spring Boot.
